...
bool SigmaEstimator::match(std::type_index transform_precision, int transform_type, int transform_dim) const {
...
std::ostream &operator<<(std::ostream &os, const SigmaEstimator &self) {
    ...
    inline_coeffs << "}";
    os << "finufft::heuristics::SigmaEstimator(" << self.type <<  "," << self.dim << "," << self.maxns << "," << inline_coeffs.str() << "," << self.lower_tol << "," << self.upper_tol << "," <<  type << ")";
    return os;
}
}
