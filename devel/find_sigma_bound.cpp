    ...
    for(auto &n_dims: cmd_opts.dim) {
        ...
        for (auto &type: cmd_opts.type) {
            ...
            for (auto &tol : tol_range) {
                ...
                if(comp_sigma>2-cmd_opts.sigma_prec) {
                    sigmas.push_back(2);
                    continue;
                }
                ...
                while (sigma_dynamic.second - sigma_dynamic.first > cmd_opts.sigma_prec) {
                    ...
                    if constexpr (is_same_v<T, double>) {
                        ...
                        finufftf_setpts(plan, cmd_opts.M, x.data(), y.data(), z.data(), cmd_opts.Ntotal, s.data(), t.data(), u.data());
                        if (type == 1 || type == 3) {
            ...
            auto polynomial = andviane::polynomial_regression<finufft::heuristics::SigmaEstimator::NCOEFFS-1>(tol_x, ups_y);
            vector<double> coeffs(polynomial.begin(), polynomial.end());

            predictors.push_back(finufft::heuristics::SigmaEstimator(type, n_dims, maxns, coeffs, lower_tol, upper_tol, typeid(T)));
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    out << "#include <finufft/heuristics.hpp>\n";
