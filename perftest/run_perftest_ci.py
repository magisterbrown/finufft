...
class Params:
    ...
    def get_hash(self) -> str:
        return hashlib.md5(json.dumps(asdict(self), sort_keys=True).encode("utf-8")).hexdigest()

    def arg_string(self) -> list[str]:
...
def gather_results(args) -> None:
    ...
    for param in PARAM_LIST:
        ...
        for transform in TRANSFORMS:
            ...
            ax.fill_between(xs, (y - std), (y + std), color=line.get_color(), alpha=0.2,)
            ...
            ax.legend()
            fig.tight_layout()
            image_path = plot_output_dir / f"params_{param.get_hash()}_type_{transform}.png"
...
def main() -> None:
    ...
    run_parser = subparsers.add_parser("run", help="Run perftest matrix and write per-tag CSV.")
    ...
    gather_parser = subparsers.add_parser("gather", help="Gather per-tag CSV files into one CSV.")
    ...
    for param in PARAM_LIST:
        ...
        for transform in TRANSFORMS:
            perftest_args = param.arg_string() + DEFAULT_EXTRA_ARGS + [f"type={transform}"]
            output = run_command(args.perftest_bin, perftest_args)
            test_res = pd.read_csv(io.StringIO("\n".join([line for line in output.splitlines() if not line.startswith("#")])), sep=",")
